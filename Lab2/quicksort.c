...
/*
int* tripartition(int *V, int start, int end, int pivot_index){
	//índices que representam o começo e o fim da partição dos números iguais
	int *partition_positions, partition_end, partition_start = start;

	//Coloca o pivô na última posição do vetor
	int aux;
	aux = V[end];
	V[end] = V[pivot_index];
	V[pivot_index] = aux;

	Percorre o vetor, comparando o elemento atual com o pivô; se for menor, troca-o com a posição
	imaginada para o pivô e aumenta a posição do pivô em uma unidade, pois achamos mais um elemento menor 
	que ele
	for (int i = start; i < end; ++i)
	{
		if (V[i] < V[end]){
			aux = V[partition_start];
			V[partition_start] = V[i];
			V[i] = aux;
	...
	//Sabemos que todos os números menores que o pivô estão em partition_start, logo é o começo da partição dos iguais.
	partition_end = partition_start;


	Percorre o vetor a partir dessa posição, e para cada número igual ao pivô, troca-o com a posição imaginada para o fim
	da partição, aumenta a posição do fim da partição em uma unidade
	for (int i = partition_end; i < end; ++i)
	{
		if (V[i] == V[end]){
			aux = V[partition_end];
			V[partition_end] = V[i];
	...
	//Troca o pivô com a posição do fim da partição dos iguais
	aux = V[end];
	V[end] = V[partition_end];
	V[partition_end] = aux;

	//Retorna as posições de começo e fim da partição
	partition_positions = (int*) malloc(sizeof(int)*2);
	partition_positions[0] = partition_start;
	partition_positions[1] = partition_end;
...
void quicksort_naive(int *V, int start, int end, int *calls){
	if (start<end){
		...
		quicksort_naive(V,start,p-1,calls);
		quicksort_naive(V,p+1,end,calls);
	}
}
...
void quicksort_naive_tripartition(int *V, int start, int end, int *calls){
	if (start<end){
		...
		quicksort_naive_tripartition(V,start,pivots[0]-1,calls);
		quicksort_naive_tripartition(V,pivots[1]+1,end,calls);
	}
}
...
void quicksort_naive_selection(int *V, int start, int end, int *calls){
	if (start<end){
		...
		quicksort_naive_selection(V, start, median-1, calls);
		quicksort_naive_selection(V, median+1, end, calls);
	}	
}
...
void quicksort_naive_selection_tripartition(int *V, int start, int end, int *calls){
	if (start<end){
		...
		quicksort_naive_selection_tripartition(V, start, partition_start-1, calls);
		quicksort_naive_selection_tripartition(V, partition_end+1, end, calls);
	}	
}
...
void quicksort_naive_insertion(int *V, int start, int end, int *calls){
	if (start<end){
		...
		if (end-start+1 <= INSERTION_LIMIT) {
			...
			quicksort_naive_insertion(V, start, p-1, calls);
			quicksort_naive_insertion(V, p+1, end, calls);
		}
	}
}
...
void quicksort_naive_pointer(int *V, int start, int end, int *calls){
	if (start<end){
		...
		quicksort_naive_pointer(V, start, p-1, calls);
		quicksort_naive_pointer(V, p+1, end, calls);
	}
}
...
void quicksort_opt1(int *V, int start, int end, int *calls){
	while (start<end){
		...
		quicksort_opt1(V,start,p-1,calls);
...
void quicksort_opt2(int *V, int start, int end, int *calls){
	...
	while(start<end){
		...
		parte */
		if ((p - start) < (end - p)){
			quicksort_opt2(V, start, p-1, calls);
			start = p+1;
		} else {
			quicksort_opt2(V, p+1, end,calls);
			end = p-1;
		}
...
void quicksort_opt2_tripartition(int *V, int start, int end, int *calls){
	...
	while(start<end){
		...
		if ((pivots[0] - start) < (end - pivots[1])){
			quicksort_opt2_tripartition(V, start, pivots[0]-1,calls);
			start = pivots[1]+1;
			free(pivots);
		} else {
			quicksort_opt2_tripartition(V, pivots[1]+1, end, calls);
			end = pivots[0]-1;
			free(pivots);
		}
	}
